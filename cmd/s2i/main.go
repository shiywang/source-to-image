...
func newCmdCompletion(root *cobra.Command) *cobra.Command {
	...
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			err := RunCompletion(&out, root, args); if err == nil {
...
func RunCompletion(out io.Writer, root *cobra.Command, args []string) error {
	if len(args) == 0 {
		return errors.UsageError(root, "Shell not specified.")
	}
	if len(args) > 1 {
		return errors.UsageError(root, "Too many arguments. Expected only the shell type.")
	}
	run, found := completion_shells[args[0]]
	if !found {
		return errors.UsageError(root, "Unsupported shell type %q.", args[0])
	}
